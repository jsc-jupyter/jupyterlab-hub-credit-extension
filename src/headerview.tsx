...
export const CreditsView: React.FC = () => {
  ...
  React.useEffect(() => {
    const start = async () => {
      console.log("hubServerUser:", hubServerUser, "hubServerName:", hubServerName, "hubPrefix:", hubPrefix);
      var evt: EventSource | null = null;
      // const token = PageConfig.getOption('token');
      if ( hubServerName && hubServerUser ) {
        evt = new EventSource(hubPrefix + 'api/credits/sseserver/' + hubServerUser + '/' + hubServerName);
      } else if ( hubServerUser ) {
        evt = new EventSource(hubPrefix + 'api/credits/sseserver/' + hubServerUser);
      } else {
        return;
      }
      // Open SSE connection
      evt.onmessage = (msg) => {
        ...
        if (data.project) {
          text += " ( " + data.project.name + ": " + data.project.balance + " / " + data.project.cap + " )";
        }      
        setCredits(text);
      };
